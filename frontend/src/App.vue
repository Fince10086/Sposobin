...
// 🌟 优化：关闭大报告。只有在模式教程从未亮起时才顺延拉起引导，手动触发关闭时则安静隐退
...
async function postIssueToBackend(sourceInfo) {
  try {
    ...
    const res = await fetch("http://localhost:8000/api/submit_issue", {
...
async function syncBackend(action_chord = null) {
  ...
  try {
    ...
    const res = await fetch("http://localhost:8000/api/sync_state", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    });
    const data = await res.json();
