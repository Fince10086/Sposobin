# engine.py - Core voice leading generation engine
...
def get_chord_candidates(chord_name, dna_db, target_s=None):
    dna = dna_db.get(chord_name, {})
    bass_candidates = dna.get("bass_options", [36, 48])
    required_classes = dna.get("required", {0, 4, 7})
...
def build_full_dag(target_melody, dna_db, key_info):
    ...
    for i in range(1, len(target_melody)):
        ...
        if not next_layer:
            # Fallback when no valid path exists
            prev_layer = layers[-2]
            fallback_cands = {}
            for nxt_c in dna_db:
...
def calculate_best_voicing(chord_sequence, initial_voicing, dna_db, key_info, target_melody=None):
    ...
    for i in range(len(chord_sequence)-1, -1, -1):
        ...
        if dp[i][curr_state][1] is not None:
